...
class Migration(DataMigration):
    depends_on = (
        ('vcsinfo', '0014_repo_add_project'),
    )

    needed_by = (
        ('vcsinfo', '0015_repo_project_not_null.py'),
    )

    def forwards(self, orm):
        for repo in orm['hmeter_frontend.vcsrepository'].objects.all():
            repo.project = repo.project_old
            repo.save()

    def backwards(self, orm):
        VCSRepository = orm['hmeter_frontend.VCSRepository']
        Repository = orm['vcsinfo.Repository']

        # Re-create VCSRepository objects (next migration drops the table)
        for base_repo in Repository.objects.filter(vcsrepository__isnull=True):
            repo = VCSRepository(project_old=base_repo.project,
                                 repository_ptr_id=base_repo)
            repo.__dict__.update(base_repo.__dict__)
            repo.save()

        # Re-populate project_old field
        for repo in VCSRepository.objects.all().exclude(
                project_old=models.F('project')):
            repo.project_old = repo.project
            repo.save()
    ...
    models = {
        ...
        u'vcsinfo.repository': {
            ...
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'last_updated': ('django.db.models.fields.DateTimeField', [], {'default': 'None', 'null': 'True', 'blank': 'True'}),
            'project': ('mptt.fields.TreeForeignKey', [], {'related_name': "'repositories'", 'null': 'True', 'to': u"orm['projectinfo.Project']"}),
    ...
    complete_apps = ['vcsinfo', 'hmeter_frontend', 'hmeter_frontend']
    symmetrical = True
