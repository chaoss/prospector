...
class Migration(SchemaMigration):
    depends_on = (
        ('gtrendsinfo', '0001_initial.py'),
    )

    def forwards(self, orm):
        # Adding model 'GTrendsQuery'
        db.create_table('hmeter_frontend_gtrendsquery', (
            ('query_ptr', self.gf('django.db.models.fields.related.OneToOneField')(to=orm['gtrendsinfo.Query'], unique=True, primary_key=True)),
            ('project', self.gf('mptt.fields.TreeForeignKey')(to=orm['projectinfo.Project'])),
        ))
        db.send_create_signal('hmeter_frontend', ['GTrendsQuery'])


    def backwards(self, orm):
        # Deleting model 'GTrendsQuery'
        db.delete_table('hmeter_frontend_gtrendsquery')
