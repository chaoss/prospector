...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for emailobj in orm['healthmeter.ParticipantEmail'].objects.all():
            values = emailobj.email.split('@', 2)
            localpart = values[0]

            try:
                domainpart = orm['healthmeter.EmailDomain'] \
                    .objects.get_or_create(domain=values[1])[0]

            except IndexError:
                domainpart = None

            emailobj.email_local = localpart
            emailobj.email_domain = domainpart
            emailobj.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        for emailobj in orm['healthmeter.ParticipantEmail'].objects.all():
            if emailobj.email_domain:
                emailobj.email = u'%s@%s' % (emailobj.email_local,
                                             emailobj.email_domain.domain)
            else:
                emailobj.email = emailobj.email_local
            emailobj.save()
