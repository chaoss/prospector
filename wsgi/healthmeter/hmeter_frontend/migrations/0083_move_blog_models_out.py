...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'BlogPost', fields ['blog', 'guid']
        db.delete_unique(u'hmeter_frontend_blogpost', ['blog_id', 'guid'])

        # Deleting model 'Blog'
        db.rename_table(u'hmeter_frontend_blog', 'bloginfo_blog')
        db.send_create_signal(u'bloginfo', ['Blog'])

        # Deleting model 'BlogPost'
        db.rename_table(u'hmeter_frontend_blogpost', 'bloginfo_post')
        db.send_create_signal(u'bloginfo', ['Post'])

        # Creating unique constraint
        db.create_unique(u'bloginfo_post', ['blog_id', 'guid'])

    def backwards(self, orm):
        # Removing unique constraint on 'BlogPost', fields ['blog', 'guid']
        db.delete_unique(u'bloginfo_post', ['blog_id', 'guid'])

        # Deleting model 'Blog'
        db.rename_table('bloginfo_blog', u'hmeter_frontend_blog')
        db.send_create_signal('hmeter_frontend', ['Blog'])

        # Deleting model 'BlogPost'
        db.rename_table('bloginfo_post', 'hmeter_frontend_blogpost')
        db.send_create_signal('hmeter_frontend', ['BlogPost'])

        # Creating unique constraint
        db.create_unique(u'hmeter_frontend_blogpost', ['blog_id', 'guid'])
