...
class Migration(SchemaMigration):
    depends_on = (
        ('jamiqinfo', '0001_initial.py'),
    )

    def forwards(self, orm):
        # Adding model 'JamiqTopic'
        db.create_table('hmeter_frontend_jamiqtopic', (
            ('topic_ptr', self.gf('django.db.models.fields.related.OneToOneField')(to=orm['jamiqinfo.Topic'], unique=True, primary_key=True)),
        ))
        db.send_create_signal('hmeter_frontend', ['JamiqTopic'])

        # Adding M2M table for field projects on 'JamiqTopic'
        db.create_table('hmeter_frontend_jamiqtopic_projects', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('jamiqtopic', models.ForeignKey(orm['hmeter_frontend.jamiqtopic'], null=False)),
            ('project', models.ForeignKey(orm['projectinfo.project'], null=False))
        ))
        db.create_unique('hmeter_frontend_jamiqtopic_projects', ['jamiqtopic_id', 'project_id'])


    def backwards(self, orm):
        # Deleting model 'JamiqTopic'
        db.delete_table('hmeter_frontend_jamiqtopic')

        # Removing M2M table for field projects on 'JamiqTopic'
        db.delete_table('hmeter_frontend_jamiqtopic_projects')
    ...
    models = {
        ...
        'hmeter_frontend.jamiqtopic': {
            'Meta': {'object_name': 'JamiqTopic', '_ormbases': ['jamiqinfo.Topic']},
            'projects': ('django.db.models.fields.related.ManyToManyField', [], {'related_name': "'jamiq_topics'", 'symmetrical': 'False', 'to': "orm['projectinfo.Project']"}),
            'topic_ptr': ('django.db.models.fields.related.OneToOneField', [], {'to': "orm['jamiqinfo.Topic']", 'unique': 'True', 'primary_key': 'True'})
        },
