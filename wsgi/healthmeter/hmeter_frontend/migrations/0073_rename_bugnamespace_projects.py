...
class Migration(SchemaMigration):
    needed_by = (
        ('btinfo', '0013_accept_bugnamespace_projects.py'),
    )

    def forwards(self, orm):
        # Removing unique constraint on 'BugNamespace_Projects', fields ['bugnamespace', 'project']
        db.delete_unique(u'hmeter_frontend_bugnamespace_projects',
                         ['bugnamespace_id', 'project_id'])

        # Deleting model 'BugNamespace_Projects'
        db.rename_table(u'hmeter_frontend_bugnamespace_projects',
                        u'btinfo_bugnamespace_projects')
        db.send_create_signal(u'btinfo', ['BugNamespace_Projects'])

        db.create_unique(u'btinfo_bugnamespace_projects',
                         ['bugnamespace_id', 'project_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'BugNamespace_Projects', fields ['bugnamespace', 'project']
        db.delete_unique(u'btinfo_bugnamespace_projects',
                         ['bugnamespace_id', 'project_id'])

        # Deleting model 'BugNamespace_Projects'
        db.rename_table(u'btinfo_bugnamespace_projects',
                        u'hmeter_frontend_bugnamespace_projects')
        db.send_create_signal(u'hmeter_frontend', ['BugNamespace_Projects'])

        db.create_unique(u'hmeter_frontend_bugnamespace_projects',
                         ['bugnamespace_id', 'project_id'])
