...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        Metric = orm['hmeter_frontend.Metric']

        # null=True for description, so just blank it out to be sure
        Metric.objects \
            .filter(description=None) \
            .update(description='')
        Metric.objects \
            .update(title=models.F('description'),
                    description='')

    def backwards(self, orm):
        "Write your backwards methods here."
        Metric.objects.update(description=models.F('title'))
