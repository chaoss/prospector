...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        dup_branches = orm['vcsinfo.branch'] \
            .objects \
            .values('name', 'repository') \
            .annotate(count=models.Count('id')) \
            .filter(count__gte=2)

        for dup_branch_data in dup_branches:
            del dup_branch_data['count']

            qs = orm['vcsinfo.branch'].objects.filter(**dup_branch_data)
            assert qs.count() > 1

            canonical_branch = qs.first()

            # ambiguous branch, so bail out
            assert not qs.exclude(
                latest_commit=canonical_branch.latest_commit).exists()

            qs.exclude(id=canonical_branch.id).delete()
